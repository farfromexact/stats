...
class AssetEvidenceTest(unittest.TestCase):
    ...
    def test_monthly_asset_evidence_can_merge_managers_and_list_manager_history(self):
        data = pd.DataFrame(
            [
                {
                    "snapshot_month": "2026-05",
                    "asset_key": "asset-a-manager-a",
                    "asset_name": "测试股票",
                    "asset_code": "000001.STK.SZ",
                    "trade_code": "000001SZ",
                    "account_bucket": "传统",
                ...
                },
                {
                    ...
                    "asset_class": "股票",
                    "manager": "Manager B",
                    "full_market_value": 0.8,
                    "avg_capital_mtd": 1.0,
                    "finance_income_mtd": 0.01,
                    "comprehensive_income_mtd": 0.03,
                },
            ]
        )

        evidence = asset_evidence(data, "2026-06", "2026-05", split_manager=False)

        self.assertEqual(len(evidence), 1)
        row = evidence.iloc[0]
        self.assertAlmostEqual(row["full_market_value_current"], 2.0)
        self.assertAlmostEqual(row["full_market_value_prior"], 1.5)
        self.assertEqual(row["current_holding_managers"], "Manager A、Manager B")
        self.assertEqual(row["ever_holding_managers"], "Manager A、Manager B、Manager C")
        self.assertNotIn("manager", evidence.columns)
